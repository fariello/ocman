...
| `ocman session export ID --to FILE` | Export a session and its subagents to a portable `.ocbox` bundle. (To export a whole project bundle, use the top-level `ocman export project SPEC to FILE`, which auto-detects session vs project.) |
...
| `ocman list running [PATTERN]` (short: `ocman lr [PATTERN]`) | List running OpenCode instances (pid/user/uptime/kind/dir/project/session) and flag insecure control servers (unauthenticated or non-loopback listeners) in bold red. Optional `PATTERN` filters (case-insensitive) by working directory, project, or attributed session (id/title/directory); no `--long` required to match session info. Observe-only; current user by default (`--all-users` opt-in). `--long` adds a best-guess (possibly stale) Session column. `--probe` confirms auth via a read-only `GET /app` on your own loopback listeners; `--json` for machine output. Fails loud if it cannot reliably enumerate (never a false "all clear"). Linux-focused. |
| `ocman doctor` | Read-only health checkup of your OpenCode storage (safe to run any time, even while OpenCode is running). Reports DB/WAL size, integrity, event-log bloat, compacted-part output, orphaned rows/diff files, old sessions, ocman + foreign backup inventory, temp leftovers, snapshots, and **listening opencode servers** (an unauthenticated control server is an ERROR, a network-exposed bind a WARN, with the same remediation as `list running`); each row names the `ocman` command that fixes it and links the upstream issue where the cause is an OpenCode bug. The server check is own-user by default (`--all-servers` widens; auth is "unknown" for other users without root), Linux-only (degrades to "unknown" elsewhere, never failing the checkup); `--deep` confirms server auth via a read-only loopback probe while `--fast` makes no network call. `--json` for machine output; `-v` for per-table/per-project detail. Never modifies anything. |
...
| `ocman spend [PROJECT]` | LLM spend report (top-level verb): per-project table by default (cost + split tokens), `PROJECT --sessions` for per-session detail, `--historical` to add saved (deleted) spend from the ledger, `--json` for machine output. |
| `ocman kill [PID\|PATTERN]` | Stop the opencode running at/under the current dir (or matching `PATTERN` by working dir / project / attributed session) WITHOUT relaunching. A bare all-digits argument is treated as a PID and killed directly (rejected before any action if it is gone, not yours, or not an opencode process). The no-relaunch counterpart to `reconnect`. One match is killed; several prompt you (incl. "all"). The confirm preview shows PID, Kind, Uptime, Project, and Session (with provenance), plus a Listener/Auth line when that instance serves a control server. Own-user only (checked via `/proc/<pid>` owner) with a PID-reuse guard; SIGTERM by default (short wait), `--force`/`-9` escalates a survivor to SIGKILL. Confirms by default; `-y` skips, `--dry-run` previews; exits non-zero if anything survived. **Linux-only.** |
...
*   **Platforms**: The database, spend, export/recovery, and TUI features work across platforms, but
    the process-based features (`kill`, `reconnect`, `list running`/`lr`, and `doctor`'s
    listening-server check) rely on `/proc` and are Linux-only, degrading to "unknown" or being
    unavailable on macOS/Windows. The `pysqlite3-binary` dependency is Linux-only (the standard
    library `sqlite3` module is used elsewhere).
