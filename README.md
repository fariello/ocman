...
| `ocman kill [PATTERN]` | Stop the opencode running at/under the current dir (or matching `PATTERN` by working dir / project / attributed session) WITHOUT relaunching. The no-relaunch counterpart to `reconnect`. One match is killed; several prompt you (incl. "all"). Own-user only (checked via `/proc/<pid>` owner) with a PID-reuse guard; SIGTERM by default (short wait), `--force`/`-9` escalates a survivor to SIGKILL. Confirms by default; `-y` skips, `--dry-run` previews; exits non-zero if anything survived. **Linux-only.** |
