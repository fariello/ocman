# opencode_recover_session.py prompt replacement recommendations, v2
...
This version corrects an important issue from the prior draft: the compaction LLM cannot ask follow-up questions because it is being called by a deterministic script. Therefore, all compaction prompts now require the model to record uncertainty, blockers, and unsafe assumptions in the output document rather than asking the user anything.
...
The future opencode agent may ask the user for clarification, but only when continuing would risk damaging work, losing data, triggering external side effects, or contradicting prior decisions.
...
- Whether the future opencode agent should continue implementation, inspect state, debug, test, document, commit, or ask before proceeding.
...
- A question the prior agent needed to answer.
...
Do not phrase these as questions to the user unless user input is genuinely required. Prefer phrasing them as items for the future agent to verify from the repository or transcript first.
...
10. The future opencode agent may request clarification from the user only if proceeding would risk damaging work, losing data, triggering external side effects, or contradicting prior decisions.
...
- No instruction for the compaction model to ask the user questions.
- Preservation of the future opencode agent's ability to ask only when continuation would create meaningful risk.
