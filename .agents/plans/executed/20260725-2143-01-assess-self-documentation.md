...
- Status: executed
- Approval: approved by maintainer 2026-07-25
...
## Execution record (2026-07-25, its_direct/pt3-claude-opus-4.8-1m-us)
Executed all 9 steps after maintainer approval. SD-01: no-args picker now TTY-gated (piped stdin
prints the listing + a next-step and exits 0; verified `ocman </dev/null; echo $?` = 0, no EOF).
SD-02: help "no args" line corrected. SD-03: `db info` on a missing DB now raises
`_db_not_found_error` (exit 1, actionable; verified). SD-04: search-empty exit-0 intent documented
in a comment. SD-05: `epilog=` examples added to session recover/compact, db clean, backup create,
search, and focus. SD-06: `filter` renamed to primary `focus` (help + dispatch + command check),
`filter` kept as a HIDDEN alias via `preprocess_argv` (verified `ocman focus`/`ocman filter` both
work and `ocman help` no longer lists the `filter` command). SD-07: `db rebase` help de-jargoned
(shown in `ocman db -h`). SD-08: `focus` option help fleshed out. SD-09: bare empty-result dies got
next-step hints. README `focus` row + CHANGELOG `[Unreleased]` entry added. Tests: renamed
`test_subcommand_filter` -> `test_subcommand_focus`, added `..._filter_is_hidden_alias_for_focus`,
`test_e2e_db_info_missing_db_exits_nonzero`, `test_e2e_no_args_non_tty_does_not_crash`; existing
picker + db-info tests still pass. Full suite: 531 passed, 2 skipped (perf benchmarks gated on
OCMAN_BENCHMARK=1). No em/en dashes introduced. Shell completion remains deferred (own IPD).
