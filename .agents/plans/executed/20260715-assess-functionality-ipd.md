...
- 2026-07-15 execute (its_direct/pt3-claude-opus-4.8): Steps 1-7 implemented and committed (bf62ddb Steps 1-4; 2324e58 Step 5; d764a7f Step 6; a10359d Step 7). Step 7 (--json) scoped to session list / project list / history show with a schema_version envelope + shared emit_json helper; search + db info JSON DEFERRED to a follow-up (nested output warrants its own schema, Medium remediation risk on functionality if rushed).
- 2026-07-15 execute (its_direct/pt3-claude-opus-4.8): Step 8 (ocman spend) implemented: per-project table default, `spend <project> --sessions` detail, `--historical` (global ledger line), `--json`; forked-spend dedupe deferred (stretch). Docs (README/CHANGELOG) synced, TODO.md item marked shipped. Full suite 303 passed, 2 skipped. All approved steps done; moving IPD to executed/.
