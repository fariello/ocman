...
- 2026-07-25 executed (its_direct/pt3-claude-opus-4.8-1m-us): approved; applied steps 1-8 to README.md, CHANGELOG.md, TODO.md. Verified against the shipped `ocman kill -h` (`[PID|PATTERN]`, all-digits=PID), `ocman lr -h` (`--long` Session column), and `ocman spend -h` (top-level verb). No em/en dashes introduced (the sole CHANGELOG dash is the pre-existing NOTICE attribution line). Doc-only; no code/tests changed.
