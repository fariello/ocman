...
- Status: executed
- Approval: approved by maintainer 2026-07-26
...
## Execution record (2026-07-26, its_direct/pt3-claude-opus-4.8-1m-us)
Executed PA-01..12 after maintainer approval. PA-01 `notice_prefix()`. PA-02/10/11/12 pending
storage layer: `pending_path()` (sibling of the config-driven `OPENCODE_HISTORY_PATH`),
`load_pending()` (tolerates missing/corrupt/unknown-schema -> [], drops malformed items),
`add_pending_item()`/`save_pending()` (lockfile-serialized read-modify-write, atomic os.replace),
`_valid_pending_item()` validation, `_make_pend_item()` builder, schema `{schema:1, items:[...]}`.
PA-03 gated the offer in `require_safe_to_mutate(..., pend_item, pend) -> str|None` (returns the
"pended" sentinel; `check_opencode_process_lock` raises the new `PendingDeferred`); the 5
out-of-scope callers pass no `pend_item` and are unchanged (37 guard/rebase/import/restore/rename/
move tests still pass). PA-04 `--pend` on the 4 delete-family commands (session delete via a single
pre-gate covering the single + batch paths; project delete; db clean / clean-orphans), each building
a structured item and catching `PendingDeferred` to exit 0. PA-05 `[NOTIC] X item(s) pending` banner
in `_run_main` (suppressed under `--json` and for the `pending` command). PA-06 `ocman pending
list/run/clear` + help/Maintain group entry. PA-07 `cli_pending`/drain: re-check running (refuse
unless `--while-running`), per item re-resolve (`db_find_session`/`db_find_project`) -> vanished =
skip+drop, else re-run the SAME delete path with re-preview + re-confirm (force=True after the single
gate; `-y` skips only the prompt), declined items kept. PA-08 TUI Pending tab + `load_pending_view()`
on mount. PA-09 TODO.md future-scope note. Docs: README section + CHANGELOG entry.

Deviation from the plan (recorded honestly): PA-08 surfaces the pending list via a dedicated TUI
Pending TAB (with count + items), NOT a separate always-visible count banner in the TUI chrome; the
CLI `[NOTIC]` line is the primary reminder. If a persistent TUI banner is wanted it is a small
follow-up.

Tests: 8 new (storage round-trip/tolerance, defer-records-no-act, guard-unchanged-without-pend-item
anti-regression, pending list, run-refuses-while-running, run-skips-vanished, clear-all, TUI Pending
tab). Full suite: 539 passed, 2 skipped (perf benchmarks gated on OCMAN_BENCHMARK=1). Defer -> NOTIC
-> list -> run(drain) flow verified end-to-end via the real CLI. No em/en dashes introduced.
